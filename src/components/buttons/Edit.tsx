...
import ModalEditPilot from "../modals/ModalEditPilo";
...
interface Props {
  id: number;
  caseType: string;
  data: Client | Airship | Flight | Pilot;
}

const Edit = ({ id, caseType, data }: Props) => {
  const [openModal, setOpenModal] = useState(false);
  const [formData, setFormData] = useState<Client | Airship | Flight | Pilot>(
    data
  );
  ...
  const { updateClients, updateAirships, updatePilots } = useStore();
      ...
      if (caseType === "airship") {
        ...
        const genericFiles = await Promise.all(
          genericImages.map(async (img: any) =>
            convertToFile(img.dataValues.image, img.dataValues.original_name)
          )
        );
