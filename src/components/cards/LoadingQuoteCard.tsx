...
import TopQuoteCard from './TopQuoteCard.astro'
import { PickAirship, type ImagesType } from './PickAirship'
import { MapCard } from './MapCard'
import { getAirshipImages, getAirshipsForInvoice } from '../../../lib/actions/airships/actions'
...
export const LoadingQuoteCard = ({tripID, airshipParamsArray} : props) => {
    const [storedAirshipData, setStoredAirshipData] = useState(null)
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const airshipIDsFromParams = airshipObjects.map(airship => airship.airshipID.toString())

            try {
                const storedData = await getAirshipsForInvoice(airshipIDsFromParams)
                setStoredAirshipData(storedData)

                const imageRequests = airshipIDsFromParams.map(id => getAirshipImages(parseInt(id)))
        ...
        fetchData()
    }, [tripID, airshipParamsArray])

    if (!clientFlight || !storedAirshipData) {
        return <div>No airships options were sent,
...
<div
		className="w-full h-[25%] sm:h-[10%] flex items-center justify-center mb-4 p-4 border-2 rounded border-solid bg-[#EFFFE3] border-[#d6e9c6]"
	>
