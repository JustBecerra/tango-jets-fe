...
export const FlightInfo = ({
	...
	const PhaseFields = () => {
		if (phase === "first") {
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
					<div className="space-y-4">
						<div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
							<h3 className="text-sm font-medium text-blue-800 mb-3">Flight Details</h3>
							<div className="space-y-2 text-sm">
								<div className="flex justify-between">
									<span className="text-gray-600">From:</span>
									<span className="font-medium text-gray-800">{formData.from || "Not selected"}</span>
								</div>
								<div className="flex justify-between">
									<span className="text-gray-600">To:</span>
									<span className="font-medium text-gray-800">{formData.to || "Not selected"}</span>
								</div>
								<div className="flex justify-between">
								...
								<div className="flex justify-between">
									...
									<span className="font-medium text-gray-800">{formData.flight_time}</span>
								</div>
								<div className="flex justify-between">
									<span className="text-gray-600">Master passenger:</span>
									<span className="font-medium text-gray-800">{formData.master_passenger || "Not specified"}</span>
								</div>
							</div>
						</div>
					</div>
					...
					<div>
						<div className="bg-white rounded-lg border border-gray-200 p-4">
							<h3 className="text-sm font-medium text-gray-700 mb-3">Aircraft Selection</h3>
			...
			return (
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
					<div className="bg-white rounded-lg border border-gray-200 p-4">
						<h3 className="text-lg font-medium text-gray-700 mb-3">Flight Summary</h3>
						<div className="space-y-3">
							...
							<div className="flex justify-between border-b border-gray-100 pb-2">
								<span className="text-gray-600">Launch Time:</span>
								<span className="font-medium">{formData.launchtime.toLocaleString()}</span>
							</div>
							<div className="flex justify-between border-b border-gray-100 pb-2">
								<span className="text-gray-600">Distance:</span>
								<span className="font-medium">{distance !== null ? `${distance} km` : "TBD"}</span>
							</div>
							<div className="flex justify-between border-b border-gray-100 pb-2">
								<span className="text-gray-600">Flight Time:</span>
								<span className="font-medium">{formData.flight_time || "TBD"}</span>
							</div>
							<div className="flex justify-between border-b border-gray-100 pb-2">
								<span className="text-gray-600">Master Passenger:</span>
								<span className="font-medium">{formData.master_passenger || "TBD"}</span>
							</div>
						</div>
					</div>
				</div>
	...
	return (
		<form onSubmit={handleSubmit} className="space-y-6">
			<div className="rounded-lg overflow-auto max-h-[60vh] p-1">{PhaseFields()}</div>
			{loading && (
