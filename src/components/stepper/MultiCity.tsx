...
export const MultiCity = ({ phase, setPhase, setShowToast }: props) => {
  ...
  const PhaseFields = () => {
    if (phase === "first") {
      return (
        <div className="h-[450px] w-[1000px] flex flex-col gap-y-16 mb-6 overflow-y-auto">
          {formData.map((elem, index) => (
            <div
              key={index}
              className="h-[400px] w-[900px] grid grid-auto-rows grid-cols-1 gap-6 sm:grid-cols-2 border-solid border-b-4 border-gray-400 pb-8"
            >
              <LocationSelector
                labelFrom="From"
                labelTo="To"
